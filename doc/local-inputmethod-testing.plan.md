...
lists contain the parent anchor plus `.Hans` while selected/history point at the
single visible `.Hans` mode, restarts Text Input menu agents, and requests
`.Hans` selection through TIS without launching the input-method host. If
