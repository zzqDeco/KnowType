...
- run `scripts/repair-inputmethod-selection.sh` when stale LaunchServices records, visible legacy `.Mode` TIS rows, missing parent enabled anchors, or stale selected/history parent rows make selection bounce back to another source;
