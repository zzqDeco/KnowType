...
- `scripts/repair-inputmethod-selection.sh` cleans stale local LaunchServices records, non-selectable parent preference rows, and visible legacy TIS modes when repeated development installs make the input menu bounce back to another source. It invokes the helper's explicit `repair-preferences` fallback and `bootstrap --select` to keep HIToolbox/history and `com.apple.inputsources` on the single user-selectable `.Hans` mode while leaving unrelated input sources untouched.
