...
Command-line activation flags remain available for compatibility and direct debugging before `IMKServer` starts: `--knowtype-install-activate` registers, enables, and selects the visible `com.knowtype.inputmethod.KnowType.Hans` input mode from the signed installed app context; `--knowtype-disable-input-source` disables KnowType records through TIS before a System Settings re-add; `--knowtype-purge-legacy` disables legacy `.Mode` sources and cleans LaunchServices state; `--knowtype-switch-away` moves away from KnowType before replacement; and the narrower register/enable/select flags support manual retries. Production install, rollback, repair, and selection scripts use `knowtype-inputsource-tool` instead of executing this IMK app. Registration waits for the parent and visible mode only on these explicit paths. The select flag prints both `select.status` and `select.current`; a `noErr` TIS status is treated as a successful best-effort request even if the helper-local current source has not changed, and callers decide whether to make that a hard failure. These CLI flags return before the app run loop starts, so registration does not initialize Rime user data. Stale `.Mode` registrations and parent-only selected/history rows are cleanup targets only. The actual per-client composition behavior lives in `KnowTypeInputController` inside `KnowTypeInputMethod`.

The app retains compatible `--knowtype-migrate-provider-profiles`,
...
required by a pre-v2 backup. Install and rollback execute the same shared
commands through `knowtype-inputsource-tool` so macOS never has to launch this
main IMK executable as a maintenance process.
...
- `scripts/install-inputmethod.sh` copies the bundle into `~/Library/Input Methods`, then uses `knowtype-inputsource-tool` to register and enable the parent anchor plus visible `.Hans` mode and repair scoped preferences.
- `scripts/repair-inputmethod-selection.sh` cleans stale local LaunchServices records, stale `.Mode` preference rows, parent-only selected/history rows, and stale legacy TIS modes when repeated development installs make the input menu show duplicates or bounce back to another source. It uses helper bootstrap and selection to restore and verify `.Hans`, and rewrites HIToolbox selected preferences only after helper-local selection is verified.
