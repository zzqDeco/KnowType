...
- Userdb frequency refresh uses Rime sync from a background provider, resolves the active schema's user dictionary name, then scans deterministically for that dictionary's `*.userdb.txt`; it never parses `.ldb` files and is not called from the synchronous key path.
