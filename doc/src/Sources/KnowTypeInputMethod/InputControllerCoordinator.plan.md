...
- schedules Rime userdb lexical refreshes through an injected refresh gate so production IMK sessions share stale-write protection across controllers; profile JSON/Markdown is staged outside the gate, then briefly published under the generation guard
