...
Each render row carries the `CandidatePanelSelection` used by keyboard/mouse commit paths, an enabled flag for hit-testing and accessibility, and a concise accessibility label. Ready AI labels include `AI 推荐`; disabled AI status labels use `AI 状态`. Pending AI render rows keep empty visible text and a spinner accessory, with `AI 状态，AI 推荐中` supplied explicitly for accessibility.
