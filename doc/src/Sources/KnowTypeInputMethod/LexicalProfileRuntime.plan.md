...
AI request construction may ask this runtime for a request-local `LexicalContextSnapshot`, which merges current Rime candidates with persisted profile terms and in-memory recent commits/selections. This read is memory-only and safe for the IMK hot path.
