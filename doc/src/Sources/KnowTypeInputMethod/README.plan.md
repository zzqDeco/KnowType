...
When a provider is configured, the IMK controller publishes raw marked text and indexed local prefix candidates immediately; provider-backed AI recommendation rows remain asynchronous. The first candidate publication does not include local fallback continuation rows. If the provider fails or returns no usable continuation, the async update keeps the AI slot unavailable instead of substituting local fallback text, so `Space` still commits the visible traditional prefix when it exists while `Tab` does not present fake AI output. Without a provider, local fallback continuations are controlled only by the local fallback preference, not by the cloud continuation toggle.
