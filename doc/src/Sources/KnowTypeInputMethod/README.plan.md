...
When a provider is configured, the IMK controller publishes local prefix candidates immediately and waits for provider-backed continuation rows. The immediate snapshot does not include local fallback continuation rows. If the provider fails or returns no usable continuation, the async update keeps continuation rows empty instead of substituting local fallback text, so `Space` still commits the visible traditional prefix while `Tab` does not present fake AI output. Without a provider, local fallback continuations are controlled only by the local fallback preference, not by the cloud continuation toggle.
