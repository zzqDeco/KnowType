...
The AppKit candidate panel is the active candidate presentation for the IMK bundle. This avoids the `IMKCandidates` failure mode where the system panel accepts data but does not become visible in some host apps. Prefix rows are rendered first, continuation rows after them, and raw input is rendered only while no suggestion exists. The panel uses native AppKit material, system colors, compact row metrics, row hit-testing, and accessibility elements. `Space` commits the visible snapshot for the current raw input; mouse click commits the same `CandidatePanelSelection` as keyboard selection and never commits disabled status rows.

When a provider is configured, the IMK controller publishes raw marked text and indexed local or native prefix candidates immediately when they are available; provider-backed AI recommendation rows remain asynchronous. The first candidate publication does not include local fallback continuation rows. If the provider fails or returns no usable continuation, the async update keeps the AI slot unavailable instead of substituting local fallback text, so `Space` still commits the visible traditional prefix when it exists while `Tab` does not present fake AI output. Ready AI remains the second shortcutable slot; pending, unavailable, and ineligible AI states are disabled status rows with muted styling, no shortcut, no hover selection, and no click commit. Without a provider, local fallback continuations are controlled only by the local fallback preference, not by the cloud continuation toggle.
...
Product commit decisions are shared through the session commit policy and coordinator: `Space` commits the current first prefix, applies a selected segment/continuation, commits a fully resolved composition, or selects a non-highlighted native prefix/full row by stable Rime index before falling through to generic native Rime space handling; `Return` commits the raw composition, `Tab` commits AI only when that slot is ready, `Option+number` commits a continuation, numeric candidate shortcuts commit full rows or apply segment rows without recomputing candidates, punctuation commits the current candidate/composition plus mapped punctuation, and `Option+R` requests explicit polish only. Native Rime suggestions keep local offline continuation rows when no provider is configured, duplicate native surface forms keep their Rime stable index, and runtime lexicon reloads replay the active raw input into the replacement conversion session. The IMK controller remains responsible for host integration details such as client lookup, marked text, insertion, palette visibility, input mode state ownership, and window anchoring, but those details now route through `InputControllerCoordinator` and small host/client seams so controller-adjacent behavior can be unit-tested without installing the input method.
