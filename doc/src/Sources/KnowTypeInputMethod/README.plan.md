...
The IMK controller marks composing text with `IMKTextInput.setMarkedText` only for inline-compatible hosts and uses commit-only composition for terminal, code editor, Electron, and JetBrains-style clients that are known to mishandle inline marked text. Candidate anchor lookup is delegated to `CandidateAnchorResolver`, which prefers fresh IMK text rects, then line-height rects, then Accessibility focused-range bounds if permission is already granted, then a same-composition scoped last usable anchor, and finally a stable safe point inside the screen visible frame. The panel no longer follows the mouse pointer when host text geometry is temporarily unavailable.
