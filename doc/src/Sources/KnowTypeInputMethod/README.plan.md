...
- lexical profile snapshots for AI recommendation that exclude Level 0/protected app commits
...
Product commit decisions are shared through the session commit policy and coordinator: `Space` commits the current first prefix or applies a selected segment/continuation before falling through to native Rime space handling, `Return` commits the raw composition, `Tab` commits AI only when that slot is ready, `Option+number` commits a continuation, numeric candidate shortcuts commit full rows or apply segment rows without recomputing candidates, punctuation commits the current candidate/composition plus mapped punctuation, and `Option+R` requests explicit polish only. Native Rime suggestions keep local offline continuation rows when no provider is configured. The IMK controller remains responsible for host integration details such as client lookup, marked text, insertion, palette visibility, input mode state ownership, and window anchoring, but those details now route through `InputControllerCoordinator` and small host/client seams so controller-adjacent behavior can be unit-tested without installing the input method.
