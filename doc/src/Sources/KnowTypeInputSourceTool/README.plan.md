...
- `repair-preferences`: explicit local development fallback used by `scripts/repair-inputmethod-selection.sh`; removes stale parent rows and stale `.Mode` rows from tracked input-source preferences, then restores only the user-selectable `.Hans` mode when `--add-active` is used. `--legacy-parent-anchor` is an explicit compatibility fallback, not the default repair shape; pair it with `diagnose-inputmethod.sh --legacy-parent-anchor` when validating that fallback.
