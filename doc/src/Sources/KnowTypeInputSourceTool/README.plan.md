...
- `repair-preferences`: explicit local development fallback used by `scripts/repair-inputmethod-selection.sh`; removes stale selected/history parent rows and stale `.Mode` rows from tracked input-source preferences, then restores the required non-selectable parent enabled anchor plus the user-selectable `.Hans` mode when `--add-active` is used. Selected and history preferences keep only `.Hans`. `--legacy-parent-anchor` is accepted as a deprecated compatibility no-op because the parent anchor is now the default enabled-state shape.
