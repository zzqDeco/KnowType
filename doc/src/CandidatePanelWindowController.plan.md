...
- anchors near the IMK client's marked-text end using `IMKTextInput.firstRect`, falls back through marked/selected range starts and ends, line-height rectangles, then pointer location when the client does not expose a usable rect
...
- uses a borderless AppKit status-level panel with `NSVisualEffectView` popover material, compact row sizing, full-width blue selected rows, and muted continuation styling to stay close to macOS native input method candidate windows
