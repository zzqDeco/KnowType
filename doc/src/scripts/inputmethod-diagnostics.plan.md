...
`scripts/repair-inputmethod-selection.sh` uses helper `purge-legacy`, `repair-preferences`, and `bootstrap --select` to remove legacy KnowType rows, restore the parent enabled anchor plus the single visible `.Hans` mode, keep selected/history preferences on `.Hans`, then restart `cfprefsd`, `TextInputMenuAgent`, and `TextInputSwitcher`. It does not start the installed input-method host and continues menu refresh/diagnostics even if helper-local `TISSelectInputSource(.Hans)` returns `paramErr/-50`. Use it after repeated local installs when stale `.Mode` rows, a missing parent anchor, or stale selected/history parent rows make the menu bounce back to Apple Pinyin or ABC. Add KnowType in System Settings if the third-party enabled list remains missing.
