...
- clean-room traditional pinyin decoding, compact segmentation, compact-prefix completion, typo normalization, and local correction ranking
...
`TraditionalInputEngine` is intentionally small in MVP. It provides a license-clean seed lexicon, multiple candidates for ambiguous pinyin such as `fangan`, compact input support for examples like `wojuedezhegefagnan`, unfinished compact-prefix completion for examples like `xianz -> 现在`, and technical-token passthrough for mixed Chinese/English input.
