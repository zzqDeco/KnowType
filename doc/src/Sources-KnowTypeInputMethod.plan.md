...
The IMK controller records recently committed prefix candidates in memory, persists them through `UserSelectionHistoryPersistence`, and passes a snapshot through `InputContext.userSelectionHistory` to both immediate local suggestions and async provider-backed suggestion loading. Persistence is serialized and merged with the latest on-disk history before write, so concurrent host-app controllers do not casually overwrite each other's selections. The ranking signal stays local; providers still receive only the normalized request fields defined by `LLMRequest`.
