...
- Keep local/no-secret provider switches explicit by clearing the draft API key and deleting stale non-local profile secrets during save only when no other saved profile still references them. Existing local OpenAI-compatible profiles keep their optional key when the draft API key is left blank.
