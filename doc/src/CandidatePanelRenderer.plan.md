...
Candidate rows are paged through `CandidatePanelPagingState`, with a default page size of 9 rows to match compact macOS-style candidate panels. `CandidatePanelState` owns the active page and moves PageDown/PageUp to the same visible row offset on the target page, clamping on short final pages; arrow navigation advances by one visible row and crosses page boundaries only when the selection moves past a page edge.
