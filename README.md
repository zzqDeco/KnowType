...
- plain punctuation: commit the current composition plus punctuation, or insert punctuation directly when there is no composition. Chinese punctuation is the default; `Option + .` toggles between Chinese and English punctuation for the current input session. Terminal, iTerm, Xcode, VS Code, and Codex desktop start with English half-width punctuation.
