...
Secret-like candidate hints are filtered before request construction; they do
not disable the whole request when other usable context remains.
...
- Change `AIRecommendationRuntime` to hard-block only secret-like raw input or
  locked prefix.
- Filter secret-like `candidateHints` and log `secret_hint_filtered`.
...
- `AI 已禁用` maps to `secret_like_text`; filtered hints use
  `secret_hint_filtered`.
