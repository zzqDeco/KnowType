...
- The helper registers the parent/mode pair and enables only the visible `.Hans` mode. This follows the component-mode shape used by Squirrel, McBopomofo, and macSKK while keeping the parent record out of user-selectable preference targets.
- The repair path now keeps HIToolbox enabled/history rows and `com.apple.inputsources` on `.Hans` only. Diagnostics still read the protected lists so stale `.Mode` rows and non-selectable parent preference rows are visible.
