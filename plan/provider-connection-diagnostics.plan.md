...
- Blank or whitespace-only diagnostic candidates fail the diagnostic instead of being treated as usable output.
...
- A blank draft API key can reuse an existing saved `secretName` value only when the saved secret still belongs to the same provider kind and endpoint credential scope.
- Remote providers that require a key fail before making a test request when no key is available.
- Invalid drafts surface specific validation errors before the diagnostic returns, while save-only validation errors remain visible.
...
- Connection-test successes preserve existing persistence errors because diagnostics do not repair failed saves or secret writes.
- Remote-to-local, remote-to-remote, and provider-kind draft changes do not reuse an existing remote secret when the draft API key is blank.
