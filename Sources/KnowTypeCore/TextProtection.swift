...
public enum TextProtection {
    ...
    public static func detectSecretLikeRanges(in text: String) -> [ProtectedRange] {
        let patterns: [(String, String)] = [
            ...
            (#"(?i)\bAuthorization\s*:\s*(?:Bearer|Basic)\s+[A-Za-z0-9._~+/=-]{8,}"#, "secret_authorization_header"),
            ...
            (#"(?i)\b(?:api[_-]?key|token|access[_-]?token|refresh[_-]?token|auth[_-]?token|password|passwd|secret|client[_-]?secret|private[_-]?key)\s*[:=]\s*['"]?[^'"\s]{4,}"#, "secret_assignment"),
