...
Provider profile 会以 JSON 保存，但不保存 API key。密钥通过 `SecretStore` 解析；macOS 实现使用 Keychain。设置 App 可编辑 OpenAI、Anthropic、Gemini、Ollama 和自定义 HTTP profile。把 profile 切换到本地或不需要密钥的 provider 时，会清空草稿 API key，并删除旧的 profile 级密钥。
