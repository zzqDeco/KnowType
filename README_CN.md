...
设置 App 会读写同一套 profile schema，可编辑 OpenAI、Anthropic、Gemini、Ollama 和自定义 HTTP profile。云端 profile 需要填写新 key，或复用已有 Keychain 密钥。远程 OpenAI 兼容 profile 必须显式填写 model ID；本地 OpenAI 兼容运行时可以留空 model，由本地 `/v1/models` 发现。自定义 HTTP profile 可不填写 API key，以支持本地代理 endpoint；如果填写 key，则会保存为 profile 级密钥。把 profile 切换到本地或不需要密钥的 provider，或把本地 OpenAI 兼容 profile 以空 API key 保存时，会清除过期的密钥引用；只有没有其他已保存 profile 继续引用旧密钥时，才会删除旧的 profile 级密钥。
